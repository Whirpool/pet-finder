...
class Profile extends UActiveRecord
{
    ...
    public function addImage()
    {
        ...
        if( Yii::app( )->user->hasState( 'avatar' ) ) {
            ...
            $path = Yii::app( )->getBasePath( )."/../images/uploads/profile/{$this->user_id}/";
            ...
            foreach( $userImages as $image) {
                if( is_file($image["path"])) {
                    if ($this->avatarStatus == self::STATUS_IMAGE){
                        unlink($path.$this->Avatar);
                        $this->Avatar = '';
                    }
                    if( rename( $image["path"], $path.$image["filename"] )) {
                        chmod( $path.$image["filename"], 0777 );
                        $this->avatarStatus = self::STATUS_IMAGE;
            ...
            Yii::app( )->user->setState( 'avatar', null );
        }
    }
