 ...
 * @version v0.2.10-dev-2014-03-21
        ...
        var parallelSupport = {
            ...
            isEventInParallelSubtree: function (state, evt, toState, scope, el) {
                ...
                if (parallelArray.length && (evt.name == '$stateChangeSuccess' || evt.name == '$viewContentLoading')) {
                    ...
                    if (!this.currentTransition.toState && !toState) {
...
//      console.log("Allowed ui-view updateView #" + elid + " toState: " + toState.name);
                return false;
            },
            ...
            getEnterTransition: function (state, stateParams, ancestorParamsChanged) {
                ...
                var inactiveState = inactiveStates[state.name];
                if (!inactiveState) {
                    console.log("getEnterTransition: " + state.name + " was not previously inactive.");
                    return null;
                }
                var paramsMatch = equalForKeys(stateParams, inactiveState.locals.globals.$stateParams, state.ownParams);
            ...
            compile: function (tElement, tAttrs, $transclude) {
                return function (scope, $element, attrs) {
                    ...
                    scope.$on('$viewContentLoading', function(evt, options) {
                        var toState = options && options.view && options.view.self;
                        updateView(false, evt, toState);
                    });
                    ...
                    function getPreviousLocals() {
                        var name = currentEl && currentEl.data('$uiViewName');
                    ...
                    function updateView(firstTime, event, toState) {
                        ...
                        var retainStickyDOM = state && $parallelState.isEventInParallelSubtree(state, event, toState, scope, $element);

                        if (retainStickyDOM || (!firstTime && previousLocals === latestLocals)) return; // nothing to do
                        ...
                        if (!firstTime && !latestLocals) {
                            console.log("Latestlocals is now " + latestLocals + " for " + name);
                        }
